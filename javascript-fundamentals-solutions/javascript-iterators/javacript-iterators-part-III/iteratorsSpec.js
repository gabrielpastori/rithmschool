...
describe("#getSongCountByArtist", function(){
  it("needs you to write some tests!", function(){
    expect(getSongCountByArtist(songs)['Adele']).to.equal(4);
    expect(getSongCountByArtist(songs)['Eminem']).to.equal(2);
    expect(getSongCountByArtist(songs)['Akon']).to.equal(1);
    expect(getSongCountByArtist(songs)['Baauer']).to.equal(1);
  });
});
